...
#include "timedata.h"
#include "txmempool.h" // eurekacoin
#include "util.h"
...
UniValue getdgpinfo(const JSONRPCRequest& request)
{
    ...
    obj.push_back(Pair("maxblocksize", (uint64_t)eurekacoinDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (uint64_t)eurekacoinDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (uint64_t)eurekacoinDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total eurekacoin balance of the wallet\n"
            "  \"stake\": xxxxxxx,           (numeric) the total eurekacoin stake balance of the wallet\n"

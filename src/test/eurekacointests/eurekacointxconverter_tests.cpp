...
void checkResult(bool isCreation, std::vector<EurekaCoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    EurekaCoinTxConverter converter(transaction, NULL);
    ExtractEurekaCoinTX eurekacoinTx;
    BOOST_CHECK(converter.extractionEurekaCoinTransactions(eurekacoinTx));
    std::vector<EurekaCoinTransaction> result = eurekacoinTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(eurekacointxconverter_tests, TestingSetup)

...
{
    CBitcoinAddress eurekacoinAddress(strEurekaCoinAddress);
    if(eurekacoinAddress.IsValid()){
        CKeyID keyid;
        eurekacoinAddress.GetKeyID(keyid);
...
bool ToEurekaCoinAddress(const std::string& strHash160, std::string& strEurekaCoinAddress)
{
    ...
    CKeyID keyid(key);
    CBitcoinAddress eurekacoinAddress;
    eurekacoinAddress.Set(keyid);
    if(eurekacoinAddress.IsValid()){
        strEurekaCoinAddress = eurekacoinAddress.ToString();
        return true;
    }
    return false;
}
